<?php /* Template Name: yur-litsam */ ?>

<?php get_header(); ?>

<title>Прием и утилизация аккумуляторов у юридических лиц - ООО Стандарт</title>
<meta name="keywords" content="аккумулятор, акб, стандарт, юрлицо, утилизация" />
<meta name="description" content="Для сбора отработавших аккумуляторов и свинцового лома с вашего предприятия у нас имеются грузовые машины с манипулятором, а также грузовики и кран." />

</head>
<body>


<div class="container bg-primary-custom p-0">
    <div class="container h-100 py-0">
        <div class="row">
            <div class="col-xl-5  justify-content-center align-self-center">
                <div class="row py-2">
                    <div class="col-sm-2 d-flex align-items-center">
                        <a href="https://akb-util.ru/">
                            <img class="mx-auto d-none d-sm-block"  src="/images/standart-ooo-logo.svg" width="45"
                                 height="45">
                        </a>
                    </div>
                    <div class="col-xl-10 col-lg-5 col-sm-10 align-items-left">
                        <div class="row pt-0 align-items-center justify-content-center">
                            <div class="col-xl-12 font-weight-bold d-none d-sm-block">ООО Стандарт</div>
                            <div class="col-xl-12 small m-0 d-none d-sm-block"> Прием и утилизация аккумуляторов и
                                свинцового лома
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-5 d-none d-lg-block d-xl-none text-center">
                        <div class="row pt-2 align-items-center justify-content-center">
                            <div class="col-12 font-weight-bold">
                                <div class="font-weight-bold mb-0 mt-2">+7 (495) 191-07-93</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 py-2 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="font-weight-bold">Москва и МО</div>
            </div>
            <div class="col-xl-2 py-2 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="text-danger-custom"><u>5 пунктов приема</u></div>
            </div>
            <div class="col-xl-3 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="row">
                    <div class="col-2 align-items-center pl-3 pr-0">
                        <img class="mx-auto img-fluid rounded-circle "  src="/images/phone-logo.svg">
                    </div>
                    <div class="col-xl-10 d-none d-xl-block justify-content-center align-self-center pl-2">
                        <div class="font-weight-bold">+7 (495) 191-07-93</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<header class="sticky-top">
    <nav class="">
        <div class="container bg-primary-custom  p-0 ">

            <div class="navbar navbar-expand-lg navbar-dark bg-dark-custom rounded border m-0">

                <a class="navbar-brand d-block d-sm-none" href="/">

                    <img class="mx-auto "  src="/images/standart-ooo-logo.svg" width="40" height="40"> &nbsp;ООО Стандарт
                </a>

                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdown"
                        aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>

                <div class="collapse navbar-collapse align-items-center justify-content-center" id="navbarNavDropdown">
                    <ul class="navbar-nav">
                        <li class="nav-item ">
                            <a class="nav-link uline" href="https://akb-util.ru/">ГЛАВНАЯ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/retail">ЧАСТНЫМ&nbsp;ЛИЦАМ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline active" href="/business">ОРГАНИЗАЦИЯМ<span
                                    class="sr-only">(текущая страница)</span></a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link uline" href="/posrednikam">ПОСРЕДНИКАМ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/pricing">ЦЕНА</a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link uline" href="/risks">РИСКИ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/contacts">КОНТАКТЫ</a>
                        </li>

                    </ul>
                </div>


            </div>
        </div>
    </nav>
</header>

<main role="main">
    <div class="container bg-primary-custom p-0 scroll">

        <div class="row px-2">
            <div class="col-12 ">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pl-3 my-0">
                        <li class="breadcrumb-item"><a href="https://akb-util.ru/">Главная</a></li>
                        <li class="breadcrumb-item"><a href="/business">Организациям</a></li>
                        <li class="breadcrumb-item"><a href="/services">Услуги</a></li>
                        
                        <li class="breadcrumb-item active" aria-current="page">Юрлицам</li>
                    </ol>
                </nav>
            </div>
        </div>

        <div class="row w-100 ml-0 pb-3 item">

            <div class="col-lg-12 px-3 pt-0 ">


                <div data-aos="zoom-in-down" data-aos-duration="300">
                    <div class="row w-100 ml-0 item">
                        <div class="col-lg-12 px-4 pb-4 pt-3">
                            <div class="row h-100 bg-white shadow panel p-0 m-0">
                                <div class="p-3 col-lg-12">
                                    <div class="pl-0 pt-3 text-left">
                                        <h1 class="text-danger-custom text-center text-xl-left">Прием и утилизация аккумуляторов у юридических лиц</h1>
                                    </div>
                                    <hr class="m-0"/>
                                    <div class="pl-0 pr-1">
                                        <p class="pt-2">Основным видом деятельности нашей компании являются сбор и утилизация отработавших свинцово-кислотных аккумуляторных батарей, свинцового лома и других видов свинец-содержащего сырья.</p> 
<p class="pt-2">
     Мы осуществляем прием в соответствии с лицензией Министерства потребительского рынка и услуг Московской области № 27 от 25 апреля 2012г., на осуществление деятельности по заготовке, хранению, переработке и реализации лома цветных и черных металлов. и лицензией Федеральной службы по надзору в сфере природопользования № 077 763 от 29.08.2016г на сбор и транспортировку отходов 2 и 3 класса опасности.</b>
</p>
<p class="pt-2">
     Компания предоставляет все необходимые документы: копии лицензий и приложений к лицензии, договор, приемо-сдаточные акты, справки об утилизации и другие документы, предусмотренные для выдачи при приёме и переработке опасных отходов.
</p>
<h4 class="text-danger-custom text-center text-xl-left">
Используем только электронные проверенные весы. Вы можете проверить их своим эталоном.</h4>
 
  <ul class="red">
            <li>Оплата производится в безналичным,&nbsp;наличном или смешанном виде</li>
            <li>Индивидуальные условия для постоянных клиентов</li>
            <li>Вывоз нашим транспортом от 100 кг</li>
          
        </ul>
 
 
<p class="pt-2">
     Для сбора отработавших аккумуляторов и свинцового лома с вашего предприятия у нас имеются грузовые машины с манипулятором, а также грузовики и кран.
</p>
<p class="pt-2">
     Собственным автотранспортом мы осуществляем забор лома свинца и старых аккумуляторов по г. Москва и Московской области и соседних областей: 
</p>
<p class="pt-2">

 
        <ul class="red">
            <li>Брянская область</li>
            <li>Владимирская область</li>
            <li>Ивановская область</li>
            <li>Калужская область</li>
        </ul>
   
        <ul class="red">
            <li>Рязанская область</li>
            <li>Смоленская область</li>
            <li>Тульская область</li>
            <li>Ярославская область</li>
        </ul>
 
 </p>
 


 
<p class="pt-2">
     Не&nbsp;забывайте какое отрицательное влияние на состояние окружающей среды, которое могут оказывать отходы и лом АКБ. И поэтому, прежде всего, мы сами должны беспокоиться о соблюдении всех норм и требований при утилизации аккумуляторов, которые устанавливаются соответствующими инстанциями занимающимися охраной окружающей среды.
</p>


                                    </div>
                                </div>

                            </div>


                        </div>
                    </div>
                </div>


            </div>


        </div>
    </div>

</main>

<?php get_footer(); ?>
