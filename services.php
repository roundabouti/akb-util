<?php /* Template Name: services */ ?>

<?php get_header(); ?>

<title>Наши услуги - ООО Стандарт</title>
<meta name="keywords" content="аккумулятор, акб, стандарт, услуги, утилизация" />
<meta name="description" content="Станьте нашим партнером по приему автомобильных аккумуляторов и начните зарабатывать" />

</head>
<body>


<div class="container bg-primary-custom p-0">
    <div class="container h-100 py-0">
        <div class="row">
            <div class="col-xl-5  justify-content-center align-self-center">
                <div class="row py-2">
                    <div class="col-sm-2 d-flex align-items-center">
                        <a href="/home">
                            <img class="mx-auto d-none d-sm-block" src="/images/standart-ooo-logo.svg" width="45"
                                 height="45">
                        </a>
                    </div>
                    <div class="col-xl-10 col-lg-5 col-sm-10 align-items-left">
                        <div class="row pt-0 align-items-center justify-content-center">
                            <div class="col-xl-12 font-weight-bold d-none d-sm-block">ООО Стандарт</div>
                            <div class="col-xl-12 small m-0 d-none d-sm-block"> Прием и утилизация аккумуляторов и
                                свинцового лома
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-5 d-none d-lg-block d-xl-none text-center">
                        <div class="row pt-2 align-items-center justify-content-center">
                            <div class="col-12 font-weight-bold">
                                <div class="font-weight-bold mb-0 mt-2 roistatphone">+7 (495) 191-07-93</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 py-2 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="font-weight-bold">Москва и МО</div>
            </div>
            <div class="col-xl-2 py-2 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="text-danger-custom"><u>5 пунктов приема</u></div>
            </div>
            <div class="col-xl-3 d-none d-xl-block justify-content-center align-self-center text-center">
                <div class="row">
                    <div class="col-2 align-items-center pl-3 pr-0">
                        <img class="mx-auto img-fluid rounded-circle " src="/images/phone-logo.svg">
                    </div>
                    <div class="col-xl-10 d-none d-xl-block justify-content-center align-self-center pl-2">
                        <div class="font-weight-bold roistatphone">+7 (495) 191-07-93</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<header class="sticky-top">
    <nav class="">
        <div class="container bg-primary-custom  p-0 ">

            <div class="navbar navbar-expand-lg navbar-dark bg-dark-custom rounded border m-0">

                <a class="navbar-brand d-block d-sm-none" href="/">

                    <img class="mx-auto " src="/images/standart-ooo-logo.svg" width="40" height="40"> &nbsp;ООО Стандарт
                </a>

                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdown"
                        aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>

                <div class="collapse navbar-collapse align-items-center justify-content-center" id="navbarNavDropdown">
                    <ul class="navbar-nav">
                        <li class="nav-item ">
                            <a class="nav-link uline" href="/home">ГЛАВНАЯ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/retail">ЧАСТНЫМ&nbsp;ЛИЦАМ</a>
                        </li>
                        <li class="nav-item dropdown">
                            	
                            	<a class="nav-link dropdown-toggle active" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          ОРГАНИЗАЦИЯМ<span
                                    class="sr-only">(текущая страница)</span></a>
        </a>
                       <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                   <a class="dropdown-item" href="https://akb-util.ru/business/">Наши преимущества</a>     	
          <a class="dropdown-item" href="https://akb-util.ru/services/">Услуги</a>
    <a class="dropdown-item" href="https://akb-util.ru/info/">Юридическая информация</a>
    <a class="dropdown-item" href="https://akb-util.ru/faq/">Вопросы и ответы</a>
    
    
        </div>     	

</li>
                        <li class="nav-item ">
                            <a class="nav-link uline" href="/posrednikam">ПОСРЕДНИКАМ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/pricing">ЦЕНА</a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link uline" href="/risks">РИСКИ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link uline" href="/contacts">КОНТАКТЫ</a>
                        </li>

                    </ul>
                </div>


            </div>
        </div>
    </nav>
</header>

<main role="main">
    <div class="container bg-primary-custom p-0 scroll">

        <div class="row px-2">
            <div class="col-12 ">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pl-3 my-0">
                        <li class="breadcrumb-item"><a href="/home">Главная</a></li>
                        <li class="breadcrumb-item"><a href="/business">Организациям</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Услуги</li>
                    </ol>
                </nav>
            </div>
        </div>

        <div class="row w-100 ml-0 pb-3 item">

            <div class="col-lg-12 px-3 pt-0 ">


                <div data-aos="zoom-in-down" data-aos-duration="300">
                    <div class="row w-100 ml-0 item">
                        <div class="col-lg-12 px-4 pb-4 pt-3">
                            <div class="row h-100 bg-white shadow panel p-0 m-0">
                                <div class="p-3 col-lg-6">
                                    <div class="pl-0 pt-3 text-left">
                                        <h3 class="text-danger-custom text-center text-xl-left">УСЛУГИ</h3>
                                    </div>
                                    <hr class="m-0"/>
                                    <div class="pl-0 pr-1">
                                        <p class="pt-2">Наши высокие цены приема позволяют нам взаимовыгодно работать с
                                            дружественными приемными пунктами.</p>

                                        <p class="pt-2"> Если вы желаете зарабатывать деньги на сборе аккумуляторного
                                            лома, то все что вам нужно сделать - позвонить нам по телефону и спросить
                                            Артема.</p>

                                        <p class="pt-2"> Имея автомобиль и место для хранения аккумуляторов (или просто
                                            стоящий без дела грузовичек) вы можете начать бизнес вместе с нами.</p>


                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <br>
                                    <div class="row">
                                        <div class="col">
                                            <div class="pl-0 pr-1"><small></small></div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col">
                                            <br>
                                            <div class="pl-0 pr-1">Мы предлагаем:</div>
                                        </div>
                                    </div>
                                    <div class="row   font-weight-bold">
                                        <div class="col">
                                            <div class="pl-0 pr-1">
                                                <ul class="red">
                                                    <li>Лучшие цены на аккумуляторный лом</li>
                                                    <li>Вывоз лома своим транспортом</li>
                                                    <li>Размещение рекламы на вашей машине</li>
                                                    <li>Размещение адреса и телефона вашей точки сбора у нас на сайте и
                                                        на карте
                                                    </li>
                                                    <li>Честные, дружественные отношения</li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row pb-3 ">
                                        <div class="col-12 ">
                                            <div class="pl-0 pr-1">Начните дополнительно зарабатывать на приеме
                                                аккумуляторов, без вложений, получая поток клиентов от нас: Станьте
                                                нашим партнером по приему автомобильных аккумуляторов и начните получать
                                                от нас дополнительный доход и клиентов! Платим в течении 3-х часов,
                                                любые объемы, предоставляем все документы!
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>


                        </div>
                    </div>
                </div>

                <div data-aos="zoom-in-down" data-aos-duration="300">
                    <div class="row w-100 ml-0 item">
                        <div class="col-lg-12 px-4 pb-4 pt-3">
                            <div class="row h-100 bg-white shadow panel p-0 m-0">
                                <div class="pt-3 px-3 col-lg-6">
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/stati-i-dokumenty/">Статьи и
                                        документы</a></h6>
                                        <hr>
                                        <h6 class="text-danger-custom text-center text-xl-left"><a
                                                href="https://akb-util.ru/services/priem-akb-ot-ibp/">Прием АКБ от
                                            ИБП</a></h6>
                                        <hr>

                                </div>

                                <div class="pt-3 px-3 col-lg-6">
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/tsena-skupki/">Цена скупки</a></h6>
                                    <hr>
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/vyvoz-akb/">Вывоз АКБ</a></h6>
                                    <hr>
                                </div>

                                <div class="px-3 pb-3 col-lg-6">
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-i-utilizatsiya-tyagovykh-akkumulyatorov/">Приём
                                        и утилизация тяговых аккумуляторов</a></h6>
                                    <hr>
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-gelevykh-akkumulyatorov/">Приём
                                        гелевых аккумуляторов</a></h6>
                                         <hr>
                                </div>

                                <div class="px-3 pb-3 col-lg-6">
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-svintsa/">Приём свинца</a></h6>
                                    <hr>
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-svintsovykh-akkumulyatorov/">Приём
                                        свинцовых аккумуляторов</a></h6>
                                         <hr>
                                </div>

                                <div class="px-3 pb-3 col-lg-6">
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-avtomobilnykh-akkumulyatorov/">Приём автомобильных аккумуляторов</a></h6>
                                    <hr>
                                    <h6 class="text-danger-custom text-center text-xl-left"><a
                                            href="https://akb-util.ru/services/priem-svintsovykh-akkumulyatorov/"> </a></h6>
                                             <hr>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>


            </div>


        </div>
    </div>

</main>

<?php get_footer(); ?>
